...
// 修改後的搜尋功能：更新 iframe 的 src 屬性來內嵌地圖結果
function searchFromInput() {
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      const mapUrl = `https://www.google.com/maps?q=${encodeURIComponent(keyword)}&ll=${lat},${lng}&z=15&output=embed`;
      document.getElementById("mapFrame").src = mapUrl;
    }, () => {
      const mapUrl = `https://www.google.com/maps?q=${encodeURIComponent(keyword)}&output=embed`;
      document.getElementById("mapFrame").src = mapUrl;
    });
  } else {
    const mapUrl = `https://www.google.com/maps?q=${encodeURIComponent(keyword)}&output=embed`;
